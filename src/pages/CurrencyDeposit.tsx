...
import { LogOut, Zap, ArrowRight, DollarSign, ArrowLeftRight, History } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-950 text-white relative overflow-hidden font-poppins">
      ...
      <div className="relative z-10 p-6 max-w-4xl mx-auto">
        ...
        >
          ...
          <p className="text-xl text-gray-400">Deposit Naira and convert to USD</p>
        </motion.div>

        <Tabs defaultValue="deposit" className="w-full">
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="deposit">Deposit Naira</TabsTrigger>
          ...
          <TabsContent value="deposit">
            <Card className="p-6 backdrop-blur-xl bg-white/5 border border-white/10">
              ...
              <div className="space-y-4">
                <div>
                  <Label htmlFor="naira_amount" className="text-white">Amount (NGN)</Label>
                  <Input
                    id="naira_amount"
                    type="number"
                    value={nairaAmount}
                  ...
                  {nairaAmount && (
                    <p className="text-sm text-gray-400 mt-2">
                      Equivalent: ${(parseFloat(nairaAmount) / exchangeRate).toFixed(2)} USD
                    </p>
                  )}
                </div>

                <div className="bg-slate-800/30 p-4 rounded-lg">
                  <h4 className="font-semibold text-white mb-2">Bank Details</h4>
                  <p className="text-sm text-gray-300">Bank: GTBank</p>
                  <p className="text-sm text-gray-300">Account: 0123456789</p>
                  <p className="text-sm text-gray-300">Name: Okdub Limited</p>
                  <p className="text-xs text-gray-400 mt-2">
                    Transfer the exact amount and your deposit will be processed within 24 hours.
                  </p>
                </div>

                <Button
                  onClick={handleNairaDeposit}
                  disabled={processingDeposit || !nairaAmount}
                  className="w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700"
                >
                  {processingDeposit ? 'Processing...' : 'Create Deposit Request'}
                    ...
                    .map((deposit) => (
                      <div key={deposit.id} className="bg-slate-800/30 p-4 rounded-lg">
                        <div className="flex justify-between items-center mb-3">
                          ...
                          <Button
                            onClick={() => handleConvertToUSD(deposit.id)}
