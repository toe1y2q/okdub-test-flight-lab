  ...
  return <div className="min-h-screen bg-slate-950 text-white relative overflow-hidden font-poppins">
      ...
      duration: 0.8
        ...
        <div className="flex items-center space-x-4">
          ...
          <Button onClick={() => navigate('/bounties')} variant="outline" size="sm" className="border-yellow-400/30 text-yellow-400 hover:bg-yellow-400/10">
            Bug Bounties
          </Button>
      ...
      <div className="relative z-10 p-6 max-w-7xl mx-auto">
        ...
        duration: 0.6
          ...
          <Button onClick={() => setShowCreateForm(!showCreateForm)} className="bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-600 hover:to-purple-700">
        ...
        y: 0
            ...
            <Card className="p-6 backdrop-blur-xl bg-white/5 border border-white/10">
              ...
              <form onSubmit={handleCreateProject} className="space-y-4">
                ...
                ...formData,
                name: e.target.value
              })} placeholder="Enter project name" className="bg-slate-800/50 border-slate-700 text-white" required />
                </div>
                <div>
              ...
              })} placeholder="Describe your project and testing requirements" className="bg-slate-800/50 border-slate-700 text-white" rows={4} />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-2 bg-violet-900 rounded-none">Project Type</label>
                    <Select value={formData.project_type} onValueChange={value => setFormData({
                  ...formData,
                  project_type: value
                })}>
                  ...
                  <div>
                    ...
                    <Input type="number" value={formData.budget} onChange={e => setFormData({
                  ...formData,
                  budget: parseFloat(e.target.value) || 0
                })} placeholder="Project budget" className="bg-slate-800/50 border-slate-700 text-white" min="0" step="0.01" />
                  </div>
                </div>
                <div className="flex space-x-4">
                  ...
                  <Button type="button" variant="outline" onClick={() => setShowCreateForm(false)} className="border-gray-600 text-gray-400 hover:bg-gray-600/10">
                    Cancel
                  </Button>
                </div>
              </form>
        ...
        {/* Projects Grid */}
        {loadingProjects ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => <Card key={i} className="p-6 backdrop-blur-xl bg-white/5 border border-white/10 animate-pulse">
          ...
          </div> : projects.length === 0 ? <Card className="p-12 backdrop-blur-xl bg-white/5 border border-white/10 text-center">
            ...
            <Button onClick={() => setShowCreateForm(true)} className="bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-600 hover:to-purple-700">
              Create Project
            </Button>
          ...
          y: -5
                ...
                <Card className="p-6 backdrop-blur-xl bg-white/5 border border-white/10 hover:border-cyan-500/30 transition-all duration-300">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className={`px-2 py-1 rounded text-xs ${project.status === 'active' ? 'bg-green-500/20 text-green-400' : project.status === 'completed' ? 'bg-blue-500/20 text-blue-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                      {project.status}
                    </span>
                  </div>
                  
                  {project.description && <p className="text-sm text-gray-400 mb-4 line-clamp-3">{project.description}</p>}
                  ...
                  <div className="space-y-3">
                    ...
                    {project.budget > 0 && <div className="flex items-center space-x-2 text-sm">
                  ...
                  <div className="mt-4 pt-4 border-t border-white/10">
                    <Button onClick={() => navigate(`/bounties?project=${project.id}`)} className="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600" size="sm">
                      View Bug Bounties
                    </Button>
                  </div>
                </Card>
